...
export default function PostTest() {
  ...
  const submit = useQuestionnaireSubmit('post');
  ...
  const handleSubmit = async (answers: { questionId: string; questionText: string; value: number }[]) => {
    ...
    try {
      ...
      notify('¡Respuestas guardadas! Buen trabajo.', 'success');
      const status =
      ...
      const next = nextRouteFromStatus({ preDone: status.preDone, postDone: true, allModulesDone: status.allModulesDone }, onboardingDone);
      setTimeout(() => navigate(next, { replace: true }), 1500);
    } catch {
      notify('No se pudo enviar el post-test. Intentalo de nuevo.', 'error');
  ...
  return (
    <QuestionnaireForm
      title="Evaluacion final (Post-test)"
