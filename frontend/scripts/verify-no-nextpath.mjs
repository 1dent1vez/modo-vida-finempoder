...
for (const lessonFile of lessonFiles) {
  ...
  const hasHardcodedNav = content.includes('nextPath=') || content.includes('overviewPath=');
  checks.push({
    name: `${relative} does not hardcode next/overview navigation`,
    ok: !hasHardcodedNav,
  });
}
...
console.log('Guard rail: no nextPath/overviewPath in lessons');
